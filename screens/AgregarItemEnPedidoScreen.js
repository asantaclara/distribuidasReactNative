...
class AgregarItemEnPedidoScreen extends React.Component {
    ...
    fetchPedido() {
        RestClient.getPedido(this.props.navigation.state.params)
            .then(json => {
                this.setState({pedido: json, isLoaded: true})
            });
    }
            ...
            if (pedido != null) {
                return (
                    <View style={styles.container}>
                        <ImageBackground
                            source={{uri: 'https://upload.wikimedia.org/wikipedia/commons/2/29/LogoUADE.png'}}
                            style={{width: window.width, height: 140}} imageStyle={{opacity: 0.2}}>
                        ...
                        <Button
                            ...
                            color="#0d47a1"
                        />
                        <ProductoPicker ref={this.ProductoPicker}
                                        onMounted={callbacks => this.ProductoPickerCallbacks = callbacks}/>
                    </View>
                )
            } else {
                return (
